	...
	plugins: [
		isProduction ? new PurgeCSSPlugin({
			paths: glob.sync([
				...
				`${current_working_dir}/html/exchange-and-return.html`
			]),
			keyframes: true
		}) : function() {}
	]
...
const frontend_folder_config = {
	...
	plugins: [
		...
		new MiniCssExtractPlugin({
			filename: `${working_dir_inverse_path}styles/css/exchange-and-return/exchange-and-return.css`,
		})
	]
};
const backend_folder_config = {
	...
	plugins: [
		...common_config.plugins,
		new MiniCssExtractPlugin({
			filename: '../../../../../static/styles/css/exchange-and-return/exchange-and-return.css',
		})
	]
};
